...
В следующем примере кода показано, как переопределить метод `OnRemovePage`.
...
Затем объекты `MyTab` можно использовать в визуальной иерархии оболочки вместо объектов `Tab`.
