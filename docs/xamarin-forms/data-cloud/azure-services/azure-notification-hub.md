...
description: Эта статья рассказывает, как использовать Центры уведомлений Azure для отправки push-уведомлений в приложения Xamarin.Forms на различных платформах.
ms.prod: xamarin
...
Интеграция Центров уведомлений Azure с мобильным приложением Xamarin.Forms осуществляется примерно так же, как и с платформенным приложением Xamarin. Подготовьте **приложение Firebase Cloud Messaging (FCM)** согласно инструкциям для консоли Firebase в разделе [Отправка push-уведомлений в приложения Xamarin.Android с помощью Центров уведомлений Azure](/azure/notification-hubs/xamarin-notification-hubs-push-notifications-android-gcm#create-a-firebase-project-and-enable-firebase-cloud-messaging). Следуя руководству по Xamarin.Android, выполните следующие действия:

1. Задайте имя для пакета Android, например `com.xamarin.notifysample`, которое используется в примере.
1. Скачайте файл **google-services.json** из консоли Firebase. Этот файл нужно будет добавить в приложение Android в одном из следующих этапов.
...
Для установки на устройствах с iOS потребуется компьютер под управлением macOS. Настройте службы push-уведомлений для Apple (APNS), следуя начальным инструкциям в разделе [Отправка push-уведомлений в приложения Xamarin.iOS с помощью Центров уведомлений Azure](/azure/notification-hubs/xamarin-notification-hubs-ios-push-notification-apns-get-started#generate-the-certificate-signing-request-file). Следуя руководству по Xamarin.iOS, выполните следующие действия:
...
## <a name="xamarinforms-application-functionality"></a>Функций приложений Xamarin.Forms
...
Пример приложения содержит файл **AppConstants.cs**, который определяет свойства, используемые платформенными проектами. В этом файле нужно указать значения из вашего Центра уведомлений Azure. В следующем коде показан файл **AppConstants.cs**:
...
1. [Переопределите FirebaseMessagingService для обработки сообщений](#override-firebasemessagingservice-to-handle-messages).
1. [Добавьте входящие уведомления в пользовательский интерфейс Xamarin.Forms](#add-incoming-notifications-to-the-xamarinforms-ui).

> [!NOTE]
> Действие сборки **GoogleServicesJson** входит в пакет NuGet для **Xamarin.GooglePlayServices.Base**. Visual Studio 2019 задает доступные действия сборки во время установки. Если действие сборки **GoogleServicesJson** не отображается, после установки пакетов NuGet перезапустите Visual Studio 2019.
...
### <a name="override-firebasemessagingservice-to-handle-messages"></a>Переопределение FirebaseMessagingService для обработки сообщений
...
1. Добавьте в свой проект пакет NuGet для **Xamarin.Azure.NotificationHubs.iOS**.
1. [Зарегистрируйтесь для получения уведомлений в APNS](#register-for-notifications-with-apns).
...
1. [Добавьте уведомления APNS в пользовательский интерфейс Xamarin.Forms](#add-apns-notifications-to-xamarinforms-ui).
...
Центры уведомлений Azure позволяют серверным приложениям отправлять уведомления устройствам на базе различных платформ. В примере показана отправка уведомлений с использованием консольного приложения **NotificationDispatcher**. Приложение включает файл **DispatcherConstants.cs**, который определяет следующие свойства:
