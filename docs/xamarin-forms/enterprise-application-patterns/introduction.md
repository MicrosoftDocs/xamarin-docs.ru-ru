...
Однако существует множество проблем, которые необходимо устранить при секционировании приложения в отдельные слабо связанные компоненты. К ним относятся следующие объекты.

- Решение о том, как обеспечить четкое разделение проблем между элементами управления пользовательского интерфейса и их логикой. Одним из наиболее важных решений при создании Xamarin.Forms корпоративного приложения является необходимость поместить бизнес-логику в файлы кода программной части или создать четкое разделение проблем между элементами управления пользовательского интерфейса и их логикой, чтобы сделать приложение более сопровождаемым и тестируемым. Дополнительные сведения см. в разделе [Model-View-ViewModel](~/xamarin-forms/enterprise-application-patterns/mvvm.md).
...
В этом руководстве содержатся рекомендации по этим вопросам и рассматриваются основные шаблоны и архитектура для создания кросс-платформенного корпоративного приложения с помощью Xamarin.Forms . Руководство предназначено для помощи с целью создания адаптируемого, сопровождаемого и тестируемого кода путем устранения распространенных Xamarin.Forms сценариев разработки корпоративных приложений и разделения проблем представления, логики представления и сущностей благодаря поддержке шаблона Model-View-ViewModel (MVVM).
...
![eShopOnContainers архитектуру высокого уровня](introduction-images/architecture.png)
...
- Мобильное приложение, разработанное на платформе Xamarin.Forms , которое поддерживает iOS, Android и универсальная платформа Windows (UWP).
...
[![Мобильное приложение eShopOnContainers](introduction-images/screenshots.png)](introduction-images/screenshots-large.png#lightbox "Мобильное приложение eShopOnContainers")
...
Дополнительные сведения об этих функциях см. в [ Xamarin.Forms документации](~/xamarin-forms/index.yml)и о [создании мобильных приложений с Xamarin.Forms помощью ](https://aka.ms/xamformsebook).
...
Классы из мобильного приложения eShopOnContainers можно использовать повторно в любом Xamarin.Forms приложении с минимальными изменениями или без них.
...
|Расширения функциональности|Содержит поведение, доступное для классов представлений.|
...
|Эффекты|Содержит `EntryLineColorEffect` класс, который используется для изменения цвета границы конкретных `Entry` элементов управления.|
...
Средства разработки и платформы для межплатформенных мобильных приложений Xamarin предоставляют комплексное решение для мобильных клиентских приложений B2E, B2B и B2C, предоставляя возможность совместного использования кода на всех целевых платформах (iOS, Android и Windows) и помогая снизить совокупную стоимость владения. Приложения могут совместно использовать свой пользовательский интерфейс и код логики приложения, сохраняя собственный внешний вид и поведение платформы.
