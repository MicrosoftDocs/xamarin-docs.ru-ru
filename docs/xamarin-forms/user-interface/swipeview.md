...
`SwipeView`доступна в Xamarin. Forms 4,4. Однако в `AppDelegate` настоящее время он экспериментальен и может использоваться только путем добавления следующей строки кода в класс в iOS, в `MainActivity` класс на Android или в `App` класс в UWP перед вызовом: `Forms.Init`
...
Кроме того `SwipeView` , [`Content`](xref:Xamarin.Forms.ContentView.Content) [`ContentView`](xref:Xamarin.Forms.ContentView) компонент наследует свойство от класса. `Content` Свойство является свойством Content `SwipeView` класса, поэтому его не нужно задавать явно.
...
Кроме того, `SwipeView` включает `Open` методы `Close` и, которые программно открывают и закрывают элементы для прокрутки соответственно.
...
Объект `SwipeView` должен определять содержимое, вокруг которого `SwipeView` выполняется оболочка, и прокрутки элементов, которые выводятся с помощью жеста прокрутки. Элементы считывания — это один или `SwipeItem` несколько объектов, помещенных в одну из четырех `SwipeView` направленных коллекций — `LeftItems`, `RightItems`, `TopItems`или `BottomItems`.

В следующем примере показано, `SwipeView` как создать экземпляр в XAML:
...
Коллекции `LeftItems`, `RightItems`, `TopItems`и `BottomItems` имеют тип `SwipeItems`. `SwipeItems` Класс определяет следующие свойства:
...
- `SwipeBehaviorOnInvoked`Тип `SwipeBehaviorOnInvoked`, который указывает, как `SwipeView` работает после вызова элемента считывания. Дополнительные сведения о поведении при прокрутке см. в разделе [поведение при прокрутке](#swipe-behavior).
...
Внешний вид каждого из `SwipeItem` них определяется сочетанием свойств `Text`, `IconImageSource`и: `BackgroundColor`

[![Снимок экрана с Свипевиев считывания элементов в iOS и Android](swipeview-images/swipeview-swipeitems.png "Свипевиев считывание элементов")](swipeview-images/swipeview-swipeitems-large.png#lightbox "Свипевиев считывание элементов")

`SwipeItem` При касании его `Invoked` событие срабатывает и обрабатывается зарегистрированным обработчиком событий. Кроме того, `Command` для свойства можно задать `ICommand` реализацию, которая будет выполняться при `SwipeItem` вызове метода.
...
События `SwipeStarted`, `SwipeChanging`и `SwipeEnded` сообщают направление прокрутки через `SwipeDirection` свойство в аргументах события. Это свойство имеет тип `SwipeDirection`, который представляет собой перечисление, состоящее из четырех элементов:
...
`SwipeItems` Класс имеет `Mode` свойство, которое указывает на результат взаимодействия при считывании. Этому свойству должно быть присвоено значение `SwipeMode` одного из членов перечисления:
...
В этом примере `SwipeView` содержимое можно прокрутить вправо, чтобы отобразить элемент считывания, который выполняется немедленно. После выполнения повторно отображается `SwipeView` содержимое.

## <a name="swipe-behavior"></a>Поведение при прокрутке

`SwipeItems` Класс имеет `SwipeBehaviorOnInvoked` свойство, которое указывает, как `SwipeView` работает после вызова элемента считывания. Этому свойству должно быть присвоено значение `SwipeBehaviorOnInvoked` одного из членов перечисления:

- `Auto`Указывает, что в режиме отображения `SwipeView` закрывается после вызова элемента считывания и в режиме выполнения после вызова `SwipeView` элемента считывания остается открытым. Это значение по умолчанию для свойства `SwipeItems.SwipeBehaviorOnInvoked`.
...
Пользовательские элементы прокрутки можно определить с `SwipeItemView` помощью типа. `SwipeItemView` Класс является производным от [`ContentView`](xref:Xamarin.Forms.ContentView) класса и добавляет следующие свойства:
...
В `SwipeItemView` этом примере объект состоит из, [`StackLayout`](xref:Xamarin.Forms.StackLayout) содержащего, [`Entry`](xref:Xamarin.Forms.Entry) и. [`Label`](xref:Xamarin.Forms.Label) После того `Entry`, как пользователь введет входные данные в, `SwipeViewItem` можно коснуться остальной части объекта, который выполняет `ICommand` объект, определяемый `SwipeItemView.Command` свойством.

## <a name="open-and-close-a-swipeview-programmatically"></a>Программное открытие и закрытие Свипевиев

`SwipeView`включает `Open` методы `Close` и, которые программно открывают и закрывают элементы прокрутки соответственно.
...
Затем `swipeView` объект можно закрыть с помощью `Close` метода:
...
> [!NOTE]
> При вызове `Close` метода запускается `CloseRequested` событие.
...
Кроме того, `Command` при определении свойства `SwipeItem` или `SwipeItemView` `CanExecute` делегата `ICommand` можно указать, чтобы включить или отключить элемент прокрутки.
