---
title: 'Наследование стилей в:::no-loc(Xamarin.Forms):::'
description: 'Стили могут наследоваться от других стилей для сокращения дублирования и включения повторного использования. В этой статье объясняется, как выполнить наследование стиля в :::no-loc(Xamarin.Forms)::: приложении.'
ms.prod: xamarin
...
ms.locfileid: "86996751"
---
# <a name="style-inheritance-in-no-locxamarinforms"></a>Наследование стилей в:::no-loc(Xamarin.Forms):::
...
Наследование стилей выполняется путем присвоения [`Style.BasedOn`](xref::::no-loc(Xamarin.Forms):::.Style.BasedOn) свойству существующего объекта [`Style`](xref::::no-loc(Xamarin.Forms):::.Style) . В XAML это достигается путем присвоения `BasedOn` свойству `StaticResource` расширения разметки, ссылающегося на ранее созданное `Style` . В C# это достигается путем присвоения `BasedOn` свойству `Style` экземпляра.

Стили, которые наследуются от базового стиля [`Setter`](xref::::no-loc(Xamarin.Forms):::.Setter) , могут включать экземпляры для новых свойств или использовать их для переопределения стилей из базового стиля. Кроме того, стили, которые наследуются от базового стиля, должны ориентироваться на тот же тип или тип, производный от типа, на который ссылается базовый стиль. Например, если базовый стиль предназначен для [`View`](xref::::no-loc(Xamarin.Forms):::.View) экземпляров, то стили, основанные на базовом стиле, могут ориентироваться на `View` экземпляры или типы, производные от `View` класса, [`Label`](xref::::no-loc(Xamarin.Forms):::.Label) например [`Button`](xref::::no-loc(Xamarin.Forms):::.Button) экземпляры и.
...
`baseStyle`Целевые [`View`](xref::::no-loc(Xamarin.Forms):::.View) экземпляры и устанавливают [`HorizontalOptions`](xref::::no-loc(Xamarin.Forms):::.View.HorizontalOptions) [`VerticalOptions`](xref::::no-loc(Xamarin.Forms):::.View.VerticalOptions) Свойства и. Параметр `baseStyle` не задается непосредственно в каких-либо элементах управления. Вместо этого `labelStyle` и `buttonStyle` наследуются от него, устанавливая дополнительные значения свойств, допускающих привязку. `labelStyle`И `buttonStyle` затем применяются к экземплярам [`Label`](xref::::no-loc(Xamarin.Forms):::.Label) и [`Button`](xref::::no-loc(Xamarin.Forms):::.Button) экземпляру путем задания их [`Style`](xref::::no-loc(Xamarin.Forms):::.NavigableElement.Style) свойств. Результат показан на следующих снимках экрана.
...
Эквивалентная страница C#, где [`Style`](xref::::no-loc(Xamarin.Forms):::.Style) экземпляры присваиваются непосредственно [`Style`](xref::::no-loc(Xamarin.Forms):::.NavigableElement.Style) свойствам необходимых элементов управления, показана в следующем примере кода:
...
`baseStyle`Целевые [`View`](xref::::no-loc(Xamarin.Forms):::.View) экземпляры и устанавливают [`HorizontalOptions`](xref::::no-loc(Xamarin.Forms):::.View.HorizontalOptions) [`VerticalOptions`](xref::::no-loc(Xamarin.Forms):::.View.VerticalOptions) Свойства и. Параметр `baseStyle` не задается непосредственно в каких-либо элементах управления. Вместо этого `labelStyle` и `buttonStyle` наследуются от него, устанавливая дополнительные значения свойств, допускающих привязку. `labelStyle`И `buttonStyle` затем применяются к экземплярам [`Label`](xref::::no-loc(Xamarin.Forms):::.Label) и [`Button`](xref::::no-loc(Xamarin.Forms):::.Button) экземпляру путем задания их [`Style`](xref::::no-loc(Xamarin.Forms):::.NavigableElement.Style) свойств.
