...
На странице " **обезьяна маустаче** " для загрузки изображения **монкэйфаце. png** используется конструктор. Затем он создает `SKCanvas` объект на основе этого растрового изображения и использует `SKPaint` `SKPath` объекты и для рисования маустаче на нем.
