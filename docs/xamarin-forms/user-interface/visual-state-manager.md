...
ms.locfileid: "82517512"
---
# <a name="xamarinforms-visual-state-manager"></a>Диспетчер визуальных состояний Xamarin. Forms

[![Скачать пример](~/media/shared/download.png) загрузить пример](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos)
...
В VSM введена концепция _визуальных состояний_. Представление Xamarin. Forms, например, `Button` может иметь несколько различных визуальных представлений в зависимости от его базового состояния &mdash; : отключено, нажато или имеет фокус ввода. Это состояния кнопки.
...
Эта группа визуальных состояний поддерживается для всех классов, производных от [`VisualElement`](xref:Xamarin.Forms.VisualElement), которые являются базовым классом [`View`](xref:Xamarin.Forms.View) для [`Page`](xref:Xamarin.Forms.Page)и.
...
Например, предположим, что на странице `Entry` есть представление, и вы хотите, `Entry` чтобы внешний вид элемента изменялся следующим образом:

- При `Entry` `Entry` отключении элемент должен иметь розовый фон.
- Элемент `Entry` должен иметь травяной фон обычно.
- При `Entry` наличии фокуса ввода должен расширять его нормальную высоту в два раза.
...
Чтобы присоединить разметку VSM `Entry` к представлению, сначала `Entry` разделяйте открывающий и конечный теги.
...
[`VisualStateGroups`](xref:Xamarin.Forms.VisualStateManager.VisualStateGroupsProperty)— Это присоединяемое свойство с возможностью привязки [`VisualStateManager`](xref:Xamarin.Forms.VisualStateManager) , определенное классом. (Дополнительные сведения о присоединенных свойствах привязки см. в статье [присоединенные свойства](~/xamarin-forms/xaml/attached-properties.md).) Таким способом `VisualStateGroups` свойство прикрепляется к `Entry` объекту.
...
Обратите внимание `VisualStateGroup` , что тег `x:Name` имеет атрибут, указывающий имя группы. `VisualStateGroup` Класс определяет `Name` свойство, которое можно использовать вместо него:

```xaml
<VisualStateGroup Name="CommonStates">
```

В одном элементе можно `x:Name` использовать `Name` один или, но не оба.

`VisualStateGroup` Класс определяет свойство с именем [`States`](xref:Xamarin.Forms.VisualStateGroup.States), которое представляет собой коллекцию [`VisualState`](xref:Xamarin.Forms.VisualState) объектов. `States`— Это _свойство_ `VisualStateGroups` Content, которое позволяет включать `VisualState` Теги непосредственно между `VisualStateGroup` тегами. (Свойства содержимого обсуждаются в статье [важный синтаксис XAML](~/xamarin-forms/xaml/xaml-basics/essential-xaml-syntax.md#content-properties).)
...
Каждый `Setter` тег указывает значение конкретного свойства, когда это состояние является текущим. Любое свойство, на которое `Setter` ссылается объект, должно поддерживаться связываемым свойством.
...
Обратите внимание, `Entry` что вторая также `DataTrigger` содержит как часть своей `Trigger` коллекции. Это приводит к `Entry` отключению, пока что что-либо не будет `Entry`введено в третий. Ниже приведена страница при запуске в iOS, Android и универсальная платформа Windows (UWP):
...
Текущее визуальное состояние — "отключено", а фон второго `Entry` — розовый на экранах iOS и Android. Реализация `Entry` UWP не позволяет задать цвет фона при `Entry` отключении.

При вводе какого-либо текста в `Entry`третьем, второй `Entry` переключается в нормальное состояние, а фон теперь заменяется на:
...
Когда вы намерены `Entry`коснуться второго, он получает фокус ввода. Он переключается в состояние "назначено" и увеличивает его высоту вдвое:
...
Обратите внимание `Entry` , что объект не удерживает травяной фон при получении фокуса ввода. Когда диспетчер визуального состояния переключается между визуальными состояниями, свойства, заданные предыдущим состоянием, задаются неопределенными. Помните, что визуальные состояния являются взаимоисключающими. "Нормальное" состояние не означает, что `Entry` включено. Это означает, что `Entry` параметр включен и не имеет фокуса ввода.

Если вы хотите, `Entry` чтобы элемент имел травяное изображение в состоянии "назначено", добавьте `Setter` другое в это визуальное состояние:
...
Теперь все `Entry` представления на этой странице реагируют так же, как и их визуальные состояния. Кроме того, обратите внимание, что состояние "Фокус" `Setter` теперь включает секунду `Entry` , которая дает каждому травяному фону также, когда он имеет фокус ввода:
...
Доступ к каждому из этих состояний можно получить с помощью группы визуального `CommonStates`состояния с именем.
...
[`Setter`](xref:Xamarin.Forms.Setter) Тип `TargetName` имеет свойство типа `string`, представляющее целевой элемент, который `Setter` будет управлять визуальным состоянием. `TargetName` Если свойство определено, `Setter` задает `Property` для элемента, определенного в: `TargetName` `Value`
...
В `Label` этом примере `label` `TextColor` свойство с именем будет иметь значение. `Red` При задании `TargetName` свойства необходимо указать полный путь к свойству в `Property`. Таким образом, чтобы задать `TextColor` свойство для `Label`, `Property` указывается как. `Label.TextColor`

> [!NOTE]
> Любое свойство, на которое `Setter` ссылается объект, должно поддерживаться связываемым свойством.

Страница **VSM with Setter (TargetName** ) в образце **[всмдемос](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos)** показывает, как задать состояние для нескольких элементов из одной группы состояний визуализации. XAML- `StackLayout` файл состоит из элемента, содержащего `Label` элемент, `Entry`и. `Button`
...
Разметка VSM прикрепляется `StackLayout`к. Существует два взаимоисключающих состояния с именами "нормально" и "нажато", с каждым состоянием, `VisualState` содержащим теги.

"Нормальное" состояние активно, если `Button` не нажата кнопка, и можно указать ответ на вопрос:
...
"Нажато" `VisualState` указывает, что при `Button` нажатии элемента его `Scale` свойство будет изменено со значения по умолчанию от 1 до 0,8. Кроме того `Entry` , `entry` `Text` свойство с именем будет иметь значение Париж. Таким образом, при нажатии на `Button` эту клавишу происходит масштабирование, которое немного меньше, а `Entry` отображает Париж. Затем, когда объект `Button` будет освобожден, он восмасштабируется до значения по умолчанию 1, а на `Entry` экран будет выведен любой введенный ранее текст.

> [!IMPORTANT]
> Пути к свойствам в настоящее время не `Setter` поддерживаются в элементах, которые задают `TargetName` свойство.

## <a name="define-your-own-visual-states"></a>Определение собственных визуальных состояний

Каждый класс, производный от `VisualElement` , поддерживает общие состояния "обычный", "Focused" и "Disabled". Кроме того `CollectionView` , класс поддерживает состояние "выбрано". На [`VisualElement`](https://github.com/xamarin/Xamarin.Forms/blob/master/Xamarin.Forms.Core/VisualElement.cs) внутреннем уровне класс обнаруживает, когда он включается или отключается, или фокус, и вызывает статический [`VisualStateManager.GoToState`](xref:Xamarin.Forms.VisualStateManager.GoToState(Xamarin.Forms.VisualElement,System.String)) метод:

```csharp
VisualStateManager.GoToState(this, "Focused");
```

Это единственный код диспетчера визуального состояния, который можно найти в `VisualElement` классе. Поскольку `GoToState` вызывается для каждого объекта, основанного на каждом классе, производном от `VisualElement`, можно использовать Диспетчер визуальных состояний с `VisualElement` любым объектом для реагирования на эти изменения.
...
На странице **проверки VSM** в образце **[всмдемос](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-vsmdemos)** показано, как использовать Диспетчер визуальных состояний в связи с проверкой входных данных. XAML- `StackLayout` файл состоит из, содержащего два `Label` элемента `Entry` `Button`:, и.
...
Разметка VSM прикрепляется `StackLayout` к ( `stackLayout`с именем). Существует два взаимоисключающих состояния с именами "Valid" и "Invalid" с каждым состоянием, содержащим `VisualState` Теги.

Если параметр `Entry` не содержит допустимый номер телефона, то текущее состояние является "недопустимым" и, следовательно, `Entry` имеет розовый фон, второй `Label` видим и `Button` отключен:
...
Обратите внимание, что файл кода программной части должен иметь только учетную запись объекта на странице, определяющей визуальные состояния, и `VisualStateManager.GoToState` вызывать для этого объекта. Это обусловлено тем, что оба визуальных состояния нацелены на несколько объектов на странице.
...
Визуальные состояния поддерживают триггеры состояния, которые являются специализированной группой триггеров, определяющих условия, [`VisualState`](xref:Xamarin.Forms.VisualState) при которых следует применять.

Триггеры состояния добавляются в [`StateTriggers`](xref:Xamarin.Forms.VisualState.StateTriggers) коллекцию [`VisualState`](xref:Xamarin.Forms.VisualState). Эта коллекция может содержать триггер одного состояния или несколько триггеров состояния. Будет [`VisualState`](xref:Xamarin.Forms.VisualState) применен, когда все триггеры состояния в коллекции активны.

При использовании триггеров состояния для управления визуальными состояниями Xamarin. Forms использует следующие правила приоритета, чтобы определить, [`VisualState`](xref:Xamarin.Forms.VisualState)какой триггер (и соответствующий) будут активны:

1. Любой триггер, производный от [`StateTriggerBase`](xref:Xamarin.Forms.StateTriggerBase).
1. [`AdaptiveTrigger`](xref:Xamarin.Forms.AdaptiveTrigger) Активируется из-за [`MinWindowWidth`](xref:Xamarin.Forms.AdaptiveTrigger.MinWindowWidth) соблюдения условия.
1. [`AdaptiveTrigger`](xref:Xamarin.Forms.AdaptiveTrigger) Активируется из-за [`MinWindowHeight`](xref:Xamarin.Forms.AdaptiveTrigger.MinWindowHeight) соблюдения условия.
...
Разметка VSM происходит в четырех местах файла XAML. `StackLayout` Именованный `mainStack` объект содержит как меню, так и содержимое, которое является `Image` элементом. Эта `StackLayout` ориентация должна иметь вертикальную ориентацию в книжной и горизонтальной ориентации в альбомном режиме:
...
Четвертый раздел разметки VSM имеет неявный стиль для самих кнопок. Эти наборы `VerticalOptions`разметки `HorizontalOptions`, и `Margin` свойства, связанные с книжной и альбомной ориентацией.

`BindingContext` Файл кода программной части задает свойство класса `menuStack` для реализации `Button` команд, а также присоединяет обработчик к `SizeChanged` событию страницы:
...
`SizeChanged` Обработчик вызывает `VisualStateManager.GoToState` для двух `StackLayout` элементов и `ScrollView` , а затем просматривает дочерние элементы `menuStack` для вызова `VisualStateManager.GoToState` `Button` элементов.
