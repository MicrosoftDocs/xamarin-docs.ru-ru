...
> [!IMPORTANT]
> `Expander` в настоящее время экспериментально и может использоваться только путем установки `Expander_Experimental` флага. Дополнительные сведения см. в статье [Флаги экспериментального режима Xamarin.Forms](~/xamarin-forms/internals/experimental-flags.md).
>
...
> [!NOTE]
> `Content` — это свойство содержимого класса `Expander`. Поэтому его не нужно явно задавать из XAML.
...
- [Демо-расширители (пример)](/samples/xamarin/xamarin-forms-samples/userinterface-expanderdemos/)
- [Xamarin.Forms Функции плавности](~/xamarin-forms/user-interface/animation/easing.md)
- [Триггеры Xamarin.Forms](~/xamarin-forms/app-fundamentals/triggers.md)
- [Xamarin.Forms Макеты с возможностью привязки](~/xamarin-forms/user-interface/layouts/bindable-layouts.md)