...
Все эти классы являются производными от `Transform` класса, который определяет `Value` свойство типа `Matrix` . Это свойство представляет текущее преобразование как `Matrix` объект. Дополнительные сведения о `Matrix` структуре см. в разделе [Преобразование матрицы](#transform-matrix).
