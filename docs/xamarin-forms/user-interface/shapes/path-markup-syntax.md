...
- *fillRule* является необязательным `Xamarin.Forms.Shapes.FillRule` , который указывает, должна ли геометрия использовать `EvenOdd` или `Nonzero` `FillRule` . `F0`Задает `EvenOdd` правило заполнения, а `F1` указывает `Nonzero` правило заполнения.
