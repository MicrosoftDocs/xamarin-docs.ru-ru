...
Xamarin.Formsне предоставляет собственные события для уведомления приложения об изменениях ориентации в общем коде. Однако [Xamarin.Essentials](~/essentials/index.md) содержит `DeviceDisplay` класс [], который предоставляет уведомления об изменении ориентации.
