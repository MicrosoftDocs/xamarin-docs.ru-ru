...
Средство не поддерживает цель `X`компиляции. Возможно, что другая версия средства поддерживает ее, или цель `X` компиляции не применяется в этой среде.
...
Архитектура в сообщении об ошибке недопустима для целевой платформы. Убедитесь, что для `--abi` параметра передана допустимая архитектура.

<a name="EM0009" />

### <a name="em0009-the-feature-x-is-not-currently-implemented-by-the-generator"></a>EM0009: Эта функция `X` в настоящее время не реализована генератором
...
Средство не может проанализировать синтаксис, `A` заданный для параметра командной строки. Правильный синтаксис см. в документации.
...
В таких случаях будут созданы только `init` два созданных селектора, оба из которых вызывают моно, но оболочка для второго не будет существовать.

<a name="EM1030" />

### <a name="em1030-method-m-is-not-generated-because-return-type-t-is-not-supported"></a>EM1030: метод `M` не создан, так как тип `T` возвращаемого значения не поддерживается.

Это **предупреждение о** том, что метод `M` будет игнорироваться (т. е. ничего не будет создано), так `T` как его возвращаемый тип не поддерживается.
...
В таких случаях будут созданы только `Increment` два созданных селектора, оба из которых вызывают моно, но оболочка для второго не будет существовать.
...
### <a name="em1041-indexed-properties-on-t-is-not-generated-because-multiple-indexed-properties-are-not-supported"></a>EM1041: индексированные свойства в `T` не создаются, так как несколько индексированных свойств не поддерживаются.
...
### <a name="em1052-element-e-is-not-generated-its-name-conflicts-with-other-elements-on-the-same-class"></a>EM1052: имя `E` элемента не создано, оно конфликтует с другими элементами того же класса.
