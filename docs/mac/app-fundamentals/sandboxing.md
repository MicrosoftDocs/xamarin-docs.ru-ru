...
3. Нажмите кнопку " **Далее** ", введите `MacSandbox` для имени проекта и нажмите кнопку " **создать** ":
...
9. Сделайте `ViewDidLoad` так, чтобы метод выглядел следующим образом:
...
2. Выберите **сертификаты, идентификаторы & профили**:
    ...
    [![Переход по ссылке Certificates, Identifiers & Profiles](sandboxing-images/sign02.png "Переход по ссылке Certificates, Identifiers & Profiles (Сертификаты, идентификаторы и профили)")](sandboxing-images/sign02-large.png#lightbox)
...
3. Затем дважды щелкните файл прав **. plist** и убедитесь, что наш **банк данных типа "ключ — значение** " и **КОНТЕЙНЕРы iCloud** соответствуют нашему идентификатору приложения, созданному ранее (например: `com.appracatappra.MacSandbox` ):
...
3. Выберите **все сообщения** в боковой панели и введите `sandbox` в поле поиска:
...
Учитывая наилучшие усилия по защите приложения Xamarin. Mac от вредоносного кода злоумышленника, для получения контроля над взаимодействием приложения с системой требуется только одна уязвимость в приложении (или в одной из используемых библиотек или платформ).
...
- **Связанные элементы** — с соответствующими назначениями приложение может иметь доступ к файлу с тем же именем, но с другим расширением. Например, документ, сохраненный как `.txt` файл и `.pdf` .
...
- Он находится в скрытом расположении в домашнем каталоге пользователя (обычно `~Library/Containers` ), и к нему можно получить доступ с помощью `NSHomeDirectory` функции (см. ниже) в приложении. Так как он находится в домашнем каталоге, каждый пользователь получит собственный контейнер для приложения.
...
> [!IMPORTANT]
> К сожалению, Xamarin. Mac еще не имеет 100% покрытия API (в отличие от Xamarin. iOS), поэтому `NSHomeDirectory` API не был сопоставлен в текущей версии Xamarin. Mac.
...
1. Приложение должно иметь возможность сохранить другую версию файла (с новым расширением). Например, экспорт `.txt` файла в `.pdf` файл. Для решения этой ситуации необходимо использовать `NSFileCoordinator` для доступа к файлу. Сначала нужно вызвать `WillMove(fromURL, toURL)` метод, переместить файл в новое расширение, а затем вызвать `ItemMoved(fromURL, toURL)` .
2. Приложению необходимо открыть основной файл с одним расширением и несколькими вспомогательными файлами с разными расширениями. Например, фильм и файл подзаголовка. Используйте `NSFilePresenter` для получения доступа к вторичному файлу. Укажите основной файл для `PrimaryPresentedItemURL` Свойства и вторичного файла в `PresentedItemURL` свойстве. При открытии главного файла вызовите `AddFilePresenter` метод `NSFileCoordinator` класса, чтобы зарегистрировать вторичный файл.

В обоих сценариях файл **info. plist** приложения должен объявлять типы документов, которые может открыть приложение. Для любого типа файлов добавьте `NSIsRelatedItemType` (со значением `YES` ) в его запись в `CFBundleDocumentTypes` массиве.
...
Следующие ограничения помещаются в `NSOpenPanel` и `NSSavePanel` при вызове их из изолированного приложения Xamarin. Mac:
...
1. **Настройте соответствующие права в приложении Xamarin. Mac, которое должно использовать закладки с областью действия безопасности** . для закладок, ограниченных областью действия приложения, задайте `com.apple.security.files.bookmarks.app-scope` для ключа прав значение `true` . Для закладок с областью действия документа задайте `com.apple.security.files.bookmarks.document-scope` для ключа прав значение `true` .
2. **Создайте закладку с областью действия безопасности** . это можно сделать для любого файла или папки, к которым пользователь предоставил доступ (через `NSOpenPanel` например,), что приложению потребуется постоянный доступ к. `public virtual NSData CreateBookmarkData (NSUrlBookmarkCreationOptions options, string[] resourceValues, NSUrl relativeUrl, out NSError error)` `NSUrl` Для создания закладки используйте метод класса.
3. **Разрешение закладки с областью безопасности** . когда приложению требуется снова получить доступ к ресурсу (например, после перезапуска), необходимо разрешить закладку в URL-адрес с областью действия безопасности. `public static NSUrl FromBookmarkData (NSData data, NSUrlBookmarkResolutionOptions options, NSUrl relativeToUrl, out bool isStale, out NSError error)` `NSUrl` Для разрешения закладки используйте метод класса.
4. **Явно уведомлять систему, к которой требуется доступ к файлу, из URL-адреса с областью действия безопасности** . Этот шаг необходимо выполнить сразу после получения URL-адреса уровня безопасности выше или, если позже потребуется восстановить доступ к ресурсу после его предоставления. Вызовите `StartAccessingSecurityScopedResource ()` метод класса, `NSUrl` чтобы начать доступ к URL-адресу с областью действия безопасности.
5. **Явно уведомлять систему о том, что вы закрыли доступ к файлу из URL-адреса с областью действия безопасности** . как можно скорее, следует информировать систему, когда приложению больше не требуется доступ к файлу (например, если пользователь закрывает его). Вызовите `StopAccessingSecurityScopedResource ()` метод класса, `NSUrl` чтобы запретить доступ к URL-адресу с областью действия безопасности.
...
- **Отправлять словари сведений о пользователях из распределенных уведомлений в другие задачи** . с помощью песочницы приложения вы не можете включить `userInfo` словарь при публикации в `NSDistributedNotificationCenter` объект для обмена сообщениями других задач.
...
3. Откройте консольное приложение (доступно в `/Applications/Utilities` ) и найдите `sandboxd` в журнале **все сообщения** о нарушениях.
...
6. Повторяйте до тех пор, пока не будут `sandboxd` устранены все нарушения.
...
- [Публикация в Магазине приложений](~/mac/deploy-test/publishing-to-the-app-store/index.md)
