...
Изначально этот раздел почти пуст, однако если выбрать элемент в **редакторе интерфейса** или **иерархии интерфейса**, раздел **свойств** будет заполнен сведениями о заданном элементе и свойствах, которые можно настроить.
...
Предпочтительным способом создания пользовательского интерфейса приложения Xamarin. Mac является Раскадровка (Дополнительные сведения см. в документации по [раскадровкам](~/mac/platform/storyboards/index.md) ). в результате любой новый проект, запущенный в Xamarin. Mac, будет использовать раскадровки по умолчанию.
...
3. Выберите **Mac**  >  **контроллер Windows**Mac:
...
6. Выберите **Mac**  >  **главное меню**Mac:
...
5. Предоставление элементов пользовательского интерфейса коду C# с помощью розеток и действий.
...
![Выбор элемента в иерархии интерфейса](xib-images/xcode15.png "Выбор элемента в иерархии интерфейса")
...
## <a name="exposing-ui-elements-to-c-code"></a>Предоставление элементов пользовательского интерфейса коду C#

Завершив проектирование внешнего вида пользовательского интерфейса в Interface Builder, необходимо предоставить элементы пользовательского интерфейса, чтобы к ним можно было получить доступ из кода C#. Для этого вы будете использовать действия и возможности.
...
Для разработчиков Xamarin. Mac это означает, что вы перетащили в файлы заглушки цели-C, которые соответствуют файлу C#, в котором нужно создать розетку или действие. Visual Studio для Mac создал файл с именем **MainWindow. h** в составе проекта Xcode оболочки, созданного для использования Interface Builder:

[![Пример h-файла в Xcode](xib-images/xcode16.png "Пример h-файла в Xcode")](xib-images/xcode16-large.png#lightbox)

Этот файл заглушек. h отражает **MainWindow.Designer.CS** , который автоматически добавляется в проект Xamarin. Mac при `NSWindow` создании нового. Этот файл будет использоваться для синхронизации изменений, внесенных Interface Builder. здесь мы создадим ваши розетки и действия, чтобы элементы пользовательского интерфейса были доступны коду C#.

#### <a name="adding-an-outlet"></a>Добавление розетки

Имея базовое представление о том, что представляют собой возможности и действия, давайте взглянем на создание розетки, чтобы предоставить элемент пользовательского интерфейса коду C#.
...
6. **Последнее действие было очень важным!** Если нужный файл не выбран, вы не сможете создавать возможности и действия, или они будут предоставлены неправильному классу в C#!
7. В **редакторе интерфейса**удерживайте нажатой клавишу **CTRL** на клавиатуре и перетащите метку, созданную выше, в редактор кода непосредственно под `@interface MainWindow : NSWindow { }` кодом:
...
Теперь рассмотрим создание действия, чтобы предоставить пользователю взаимодействие с элементом пользовательского интерфейса в коде C#.

Выполните следующие действия.

1. Убедитесь, что все еще находится в **редакторе помощника** , а файл **MainWindow. h** отображается в **редакторе кода**.
2. В **редакторе интерфейса**удерживайте нажатой клавишу **CTRL** на клавиатуре и перетащите кнопку, созданную выше, в редактор кода непосредственно под `@property (assign) IBOutlet NSTextField *ClickedLabel;` кодом:
...
После того, как пользовательский интерфейс разXcode и предоставляется коду C#, переключитесь обратно на Visual Studio для Mac и позвольте ему синхронизировать изменения из и Interface Builder.
...
Обратите внимание, что `NSLabel` доступ к предоставляется в C# по непосредственному имени, которое вы назначили в Xcode при создании розетки в Xcode, в данном случае он называется `ClickedLabel` . Можно получить доступ к любому методу или свойству предоставленного объекта так же, как и к любому обычному классу C#.
...
Затем добавьте в класс следующий разделяемый класс `MainWindow` :
...
Ключевая строка здесь означает `[Export ("openDocument:")]` , `NSMenu` что **AppDelegate** имеет метод `void OpenDialog (NSObject sender)` , отвечающий на `openDocument:` действие.
...
При выборе **MainWindow.Designer.CS** в **панель решения** вы сможете увидеть, как наша розетка и действия были привязаны в коде C#:
...
![Выполнение примера приложения](xib-images/new04.png "Выполнение примера приложения")
...
Измените файл XIB в окне, который будет использовать представление и отобразит его. Добавьте **пользовательское представление** , которое будет действовать в качестве контейнера для представления после его загрузки в память кодом C# и предоставления его вызываемой розетке `ViewContainer` :
...
При этом создается новый экземпляр контроллера представления для отображения нового представления, устанавливается его тип (как указано в перечислении, добавленном в проект) и используется `DisplaySubview` метод, добавленный в класс окна для фактического отображения представления. Пример:

[![Выполнение примера приложения](xib-images/view04.png "Выполнение примера приложения")](xib-images/view04-large.png#lightbox)
...
В этой статье подробно рассматривается работа с файлами. XIB в приложении Xamarin. Mac. Мы видели различные типы и использование файлов. XIB для создания пользовательского интерфейса приложения, создания и обслуживания XIB-файлов в Interface Builder Xcode и работы с XIB файлами в коде C#.
...
- [Диалоги](~/mac/user-interface/dialog.md)
