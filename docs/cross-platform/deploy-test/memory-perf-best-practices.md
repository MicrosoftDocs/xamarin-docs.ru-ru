...
Сборщик мусора SGen используется в Xamarin.iOS 9.2.1 и более поздних версиях по умолчанию, поэтому он будет применяться автоматически. Обратите внимание, что возможность изменения сборщика мусора была удалена из новых версий Visual Studio. Дополнительные сведения см. в статье [Новая система подсчета ссылок](~/ios/internals/newrefcount.md).
...
![Параметры компоновщика для Xamarin.iOS](memory-perf-best-practices-images/linker-options-ios.png)

На приведенном ниже снимке экрана показаны параметры компоновщика для проекта Xamarin.Android в Visual Studio для Mac:

![Параметры компоновщика для Xamarin.Android](memory-perf-best-practices-images/linker-options-droid.png)
