...
[![Немецкая и японская длина строки](localization-images/language-compare-sml.png)](localization-images/language-compare.png#lightbox)
...
Это также может повлиять на внешние ссылки на аудио-дорожки или видео. при проектировании приложения необходимо заранее спланировать содержимое, переведенное с помощью источников, или убедиться, что пользователи должны быть осведомлены о пользовательском интерфейсе, когда содержимое не будет представлено на их языке.
...
Обычно не является проблемой с мобильными приложениями, так как обычно строки неформатированы. Однако если в приложении требуется форматированный текст (например, полужирное начертание или курсив), убедитесь, что переводчик знает, как вводить форматирование, файлы строк сохраняют его правильно и отформатированы должным образом перед отображением пользователю (Internet Explorer). не следует случайно предоставлять пользователю коды форматирования.
...
Избегайте использования строк, таких как `"You have {0} message/s."` использование конкретных строк для каждого состояния, чтобы обеспечить лучшее взаимодействие с пользователем:
...
Существует также более очевидный случай, даже на английском языке, где строки ссылаются на конкретного пользователя или пользователя приложения. Например, некоторые сайты отображают такие сообщения, как, `"Bob commented on his post"` например, для пол, женщина, недвоичного или неизвестного пола:
