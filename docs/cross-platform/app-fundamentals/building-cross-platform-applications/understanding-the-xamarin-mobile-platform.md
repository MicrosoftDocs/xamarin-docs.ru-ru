...
Хотя Xamarin позволяет создавать приложения на языке C# и совместно использовать один и тот же код на нескольких платформах, фактическая реализация в каждой системе очень различается.

## <a name="compilation"></a>Компиляция

Источник C# делает свой подход к собственному приложению очень разными способами на каждой платформе:
...
Прелесть Xamarin заключается в том, что несмотря на различия внутри, Xamarin. iOS и Xamarin. Android (в сочетании с пакетами Microsoft Windows SDK) предлагают простой интерфейс для написания кода C#, который можно использовать повторно на всех трех платформах.
...
Платформа Xamarin позволяет повторно использовать существующий код C# на всех платформах, а также интеграцию библиотек, написанных изначально для каждой платформы.
