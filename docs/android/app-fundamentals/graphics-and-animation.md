...
- **Рисованные анимации** &ndash; Android также поддерживает анимацию по кадрам, известную как *нарисованная анимация*. Это самый простой API анимации. Android последовательно загружает и отображает отображаемые ресурсы в последовательности (во многом похоже на мультипликационный рисунок).

- **Просмотр анимаций** &ndash; *Анимация представлений* — это исходный API анимации в Android, который доступен во всех версиях Android. Этот API ограничен тем, что он будет работать только с объектами представления и может выполнять только простые преобразования этих представлений.
    Анимации представления обычно определяются в XML-файлах, находящихся в `/Resources/anim` папке.

- **Анимация свойств** &ndash; В Android 3,0 появился новый набор API анимации, известный как *анимация свойств*. В этих новых API реализована расширяемая и гибкая система, которая может использоваться для анимации свойств любого объекта, а не только для просмотра объектов. Такая гибкость позволяет инкапсулировать анимацию в отдельные классы, упрощающие совместное использование кода.
...
Получить объект можно двумя способами `Canvas` . Первый способ заключается в определении объекта [Bitmap](xref:Android.Graphics.Bitmap) , а также при создании экземпляра `Canvas` объекта с ним. Например, следующий фрагмент кода создает новый холст с базовым точечным рисунком:
...
- `alpha`Эффект &ndash; затухания или затухания.
...
[Обжектаниматор](xref:Android.Animation.ObjectAnimator) является подклассом `ViewAnimator` , объединяющим ядро времени и вычисление значений `ValueAnimator` с логикой, необходимой для подключения обработчиков событий. `ValueAnimator` &ndash; `ObjectAnimator` Для выполнения этого шага необходимо, чтобы приложения явно подстроили обработчик событий.
...
Нарисованные ресурсы определяются в XML-файле, который содержит `<animation-list>` элемент в качестве корневого элемента и ряд `<item>` элементов, определяющих каждый кадр в анимации. Этот XML-файл хранится в `/Resource/drawable` папке приложения. Следующий код XML является примером создаваемой анимации:
...
- [Демонстрация анимации (пример)](/samples/xamarin/monodroid-samples/animationdemo)
...
- [Элемент Canvas](xref:Android.Graphics.Canvas)
