...
- Измените **Resources/Values-V21/Styles. XML** и добавьте `style` узел с именем пользовательского стиля представления. Задайте атрибуты пользовательского цвета для представления в этом `style` узле. Например, чтобы создать пользовательский стиль [кардвиев](~/android/user-interface/controls/card-view.md) с более скругленными углами и использовать `my_blue` его в качестве цвета фона карты, добавьте `style` узел в **styles. XML** (внутри `resources` узла) и настройте цвет фона и радиус угла.
