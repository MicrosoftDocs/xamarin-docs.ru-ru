...
Дополнительные сведения об установке пакетов NuGet см. [в разделе Пошаговое руководство. Включение NuGet в проект](/visualstudio/mac/nuget-walkthrough).
...
Этот снимок экрана взят из примера приложения [рециклервиевер](/samples/xamarin/monodroid-samples/android50-recyclerviewer) , который использует мини-приложение `RecyclerView` для представления прокручиваемого списка `CardView` изображений для просмотра фотографий. Дополнительные сведения о `RecyclerView` см. в разделе [RecyclerView](~/android/user-interface/layouts/recycler-view/index.md) Guide.
...
- [RecyclerView (пример)](/samples/xamarin/monodroid-samples/android50-recyclerviewer)
