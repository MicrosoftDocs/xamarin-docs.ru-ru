...
Наконец, выполните выборку, переопределив `PerformFetch` метод в `AppDelegate` и передав *обработчик завершения*. Обработчик завершения — это делегат, который принимает `UIBackgroundFetchResult` :
...
В приведенном выше примере кода мы позволяем операционной системе решить, как часто следует выбирать новое содержимое, установив для параметра Интервал минимальной выборки значение `BackgroundFetchIntervalMinimum` . iOS предлагает три варианта интервала выборки:
