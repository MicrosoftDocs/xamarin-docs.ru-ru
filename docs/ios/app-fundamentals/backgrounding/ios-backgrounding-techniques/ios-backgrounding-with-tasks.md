...
1. **Задачи дидентербаккграунд** — вызываются во время выполнения `DidEnterBackground` метода жизненного цикла приложения, чтобы упростить очистку и сохранение состояния.
...
Безопасность в фоновом режиме и `DidEnterBackground` задачи можно использовать как в iOS 6, так и в iOS 7 с некоторыми незначительными отличиями. Рассмотрим эти два типа задач более подробно.
...
Процесс регистрации связывает задачу с уникальным идентификатором, `taskID` а затем заключает ее в соответствующие `BeginBackgroundTask` `EndBackgroundTask` вызовы и. Чтобы создать идентификатор, мы вызываем `BeginBackgroundTask` метод для `UIApplication` объекта, а затем запускаем долгосрочную задачу, обычно в новом потоке. После завершения задачи мы вызываем `EndBackgroundTask` и передаем тот же идентификатор. Это важно, так как iOS завершит работу приложения, если `BeginBackgroundTask` у вызова нет соответствующего запроса `EndBackgroundTask` .
...
Начнем с переопределения `DidEnterBackground` метода в `AppDelegate` , где мы регистрируем задачу с помощью, `BeginBackgroundTask` как мы делали в предыдущем примере. Затем мы порожден новый поток и выполним выполнение нашей длительной задачи. Обратите внимание, что `EndBackgroundTask` теперь вызов выполняется в долгосрочной задаче, так как `DidEnterBackground` метод уже будет возвращен.
...
Если приложение с зарегистрированными задачами перемещается в фоновый режим, для выполнения зарегистрированных задач потребуется около 600 секунд. Мы можем проверить, сколько времени задача должна завершиться с помощью свойства static `BackgroundTimeRemaining` `UIApplication` класса. Следующий код даст нам время в секундах, которое осталось в фоновой задаче:
...
Вспомним, что до iOS 7 задача, выполняемая в фоновом режиме, составила 600 секунд. Одна из причин этого ограничения заключается в том, что задача, выполняемая в фоновом режиме, будет удерживать устройство в спящий режим в течение выполнения задачи:
 ...
 [![График задачи, которая сохраняет приложение в спящем режиме до 7](ios-backgrounding-with-tasks-images/ios6.png)](ios-backgrounding-with-tasks-images/ios6.png#lightbox)

Фоновая обработка iOS 7 оптимизирована для более длительного времени работы от аккумулятора. В iOS 7 фоновый режим становится уступающей: вместо отключения устройства в спящем режиме задачи учитывают, когда устройство переходит в режим сна, а обрабатываются блоками, когда устройство выходит из спящего режима для обработки телефонных звонков, уведомлений, входящих сообщений электронной почты и других распространенных прерываний. На следующей схеме представлено представление о том, как может быть нарушена задача:

 [![График задачи, разбитой на блоки после iOS 7](ios-backgrounding-with-tasks-images/ios7.png)](ios-backgrounding-with-tasks-images/ios7.png#lightbox)
...
Чтобы выполнить передачу по сети, допустимую как в предварительных, так и после iOS 7, проверьте, доступен ли объект `NSURLSession` для передачи в очередь, и используйте обычную фоновую задачу для выполнения переноса, если это не так:
