...
Используйте `Items` свойство объекта `UITabBar` для доступа к коллекции, `UITabBarItems` содержащей нуль-индексный массив (0). `SelectedItem`Свойство вернет текущую выбранную вкладку (Category) как `UITabBarItem` .
