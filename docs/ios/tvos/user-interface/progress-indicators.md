...
3. На вкладке **мини** -приложение **панель свойств**можно настроить несколько свойств индикатора действия, например его **стиль**, **поведение**и **имя**: 
    ...
    **Имя** определяет имя свойства, которое представляет индикатор действия в коде C#.
...
В любой момент можно увидеть, является ли индикатор действия анимацией, проверив его `IsAnimating` свойство. Если `HidesWhenStopped` свойство имеет значение `true` , то индикатор действия будет автоматически скрыт при остановке его анимации.
...
`Progress`Свойство используется для задания объема задачи, которая была выполнена от 0% до 100% (0,0 до 1,0). Используйте `ProgressTintColor` свойство для задания цвета линейки завершено и `TrackTintColor` свойства, чтобы задать цвет фона (незавершенный объем).
