...
- Добавьте [`[Preserve]`](http://www.go-mono.com/docs/index.aspx?link=T:MonoTouch.Foundation.PreserveAttribute) атрибут к элементу.  Это предотвратит удаление компоновщиком.
- При вызове [**mtouch**](http://www.go-mono.com/docs/index.aspx?link=man:mtouch%281%29)используйте параметры **-Link** или **-линксдконли** .
  ...
  - **-линксдконли** будет связывать только предоставляемые Xamarin. iOS сборки, такие как **xamarin.ios.dll**, сохраняя все типы в сборках, созданных пользователями (например, в проектах приложений).
...
Эта ошибка возникает, если вы определяете класс в файлах конструктора интерфейсов, но не предоставляете фактическую реализацию для него в коде C#.
...
Проблема заключается в том, что Mono берет на себя OS X `libsqlite3.dylib` , а не `libsqlite3.dylib` файл iPhoneSimulator. Приложение *будет* работать на устройстве, но только не в симуляторе.
...
## <a name="systemexecutionengineexception-attempting-to-jit-compile-method-wrapper-managed-to-managed-foosystemcollectionsgenericicollection1get_count-"></a>System.ExeКутионенгиниксцептион: попытка выполнить JIT-компиляцию метода foo []: System. Collections. Generic. ICollection ' 1. get_Count ().
...
Это можно решить, открыв параметры для проекта Xamarin. iOS, перейдя на **iOS Build**  >  **международную языковую** сборку iOS и проверив **Запад** .
...
Если это произошло, исключите mlaunch.exe из антивирусного сканера, чтобы предотвратить повторные возникновение. Дополнительные сведения см. в статьях [Создание исключения приложения в менеджер Endpoint Protection симантекс](https://knowledge.broadcom.com/external/article/180778/how-to-create-an-application-exception-i.html) для Symantec и [исключение файлов и папок из Norton Auto-Protect, Sonar и сканирование аналитики](https://support.norton.com/sp/en/uk/home/current/solutions/v3672136) для Norton. Кроме того, можно сообщить о ложных срабатываниях в [Symantec](https://symsubmit.symantec.com) или [Norton](https://submit.norton.com/?type=FP).

После добавления исключения для mlaunch.exe потребуется повторная установка для восстановления отсутствующих файлов. Самый простой способ сделать это — переключить каналы в обновлении:
