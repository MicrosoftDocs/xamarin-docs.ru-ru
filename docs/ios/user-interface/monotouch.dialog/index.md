...
Очень касание. диалоговое окно, называемое MT. D — это быстрый набор средств разработки пользовательского интерфейса, позволяющий разработчикам создавать экраны приложений и переходы с помощью информации, а не долгой создание контроллеров представления, таблиц и т. д. Таким образом, он обеспечивает значительное упрощение разработки пользовательского интерфейса и сокращения кода. Например, рассмотрим следующий снимок экрана:
 ...
 [![Например, рассмотрим этот снимок экрана](images/image1.png)](images/image1.png#lightbox)
...
- **Расширенное использование** . здесь рассматриваются такие расширенные функции, как извлечение, поиск, Загрузка фоновых изображений, использование LINQ для построения иерархий элементов и создание пользовательских элементов, ячеек и контроллеров для использования с MT. D. 
...
*Диалогвиевконтроллер*или *DVC* для Short наследует от и, `UITableViewController` следовательно, представляет экран с таблицей. DVCs может быть отправлен на контроллер навигации так же, как обычный Уитаблевиевконтроллер.
...
Раздел — это группа ячеек в таблице. Как и в случае с обычным разделом таблицы, он может иметь верхний и нижний колонтитулы, которые могут быть либо текстовыми, либо даже пользовательскими представлениями, как показано на следующем снимке экрана:
 ...
 [![Как и в случае с обычным разделом таблицы, он может иметь верхний и нижний колонтитулы, которые могут быть либо текстовыми, либо даже пользовательскими представлениями, как на этом снимке экрана.](images/image2.png)](images/image2.png#lightbox)
...
Элемент представляет фактическую ячейку в таблице. Машин. D упакован с широким спектром элементов, представляющих различные типы данных или различные входные данные. Например, на следующих снимках экрана показаны некоторые из доступных элементов:
 ...
 [![Например, на снимках экрана показаны некоторые из доступных элементов](images/image3.png)](images/image3.png#lightbox)
...
Если RootElement инициализируется с помощью значения Section/element, это значение используется для наведения дочернего элемента, который предоставит сводку по конфигурации, которая отображается в правой части экрана. Например, на следующем снимке экрана показана таблица слева с ячейкой, содержащей заголовок экрана сведений справа, "десерт", а также значение выбранной программы.
 ...
 На [ ![ этом снимке экрана слева находится таблица с заголовком экрана сведений справа, десерт, а также значение выбранной на](images/image4.png)](images/image4.png#lightbox) снимке экрана, где [ ![ показана таблица слева с ячейкой, содержащей заголовок экрана сведений справа, десерт, а также значение выбранной](images/image5.png)](images/image5.png#lightbox) автопередачи.
...
Вызов метода Add или использование синтаксиса инициализатора C# 4 добавляет разделы.
...
Машин. D предоставляет `NSAction` как делегат для обработки обратных вызовов.
Например, предположим, что требуется выполнить обработку события касания для ячейки таблицы, созданной MT. D. При создании элемента с помощью MT. D, просто укажите функцию обратного вызова, как показано ниже:
...
Этот код создает пользовательский интерфейс, как показано ниже. Полное пошаговое руководство по этому примеру см. в разделе [Пошаговое руководство по API элементов](~/ios/user-interface/monotouch.dialog/elements-api-walkthrough.md) .
 ...
 [![В сочетании со свойством element. Value функция обратного вызова может получить значение, заданное в других элементах.](images/image6.png)](images/image6.png#lightbox)
...
A `StringElement` показывает заголовок слева от ячейки таблицы и строковое значение в правой части ячейки.
 ...
 [![Стринжелемент отображает заголовок слева от ячейки таблицы и строковое значение в правой части ячейки.](images/image7.png)](images/image7.png#lightbox)
...
```
 ...
 [![Чтобы использовать Стринжелемент в качестве кнопки, укажите делегат](images/image8.png)](images/image8.png#lightbox)
...
`StyledStringElement`Позволяет представить строки с помощью встроенных стилей ячеек таблицы или пользовательского форматирования.
 ...
 [![Стиледстринжелемент позволяет представлять строки либо с помощью встроенных стилей ячеек таблицы, либо с помощью пользовательского форматирования.](images/image9.png)](images/image9.png#lightbox)

`StyledStringElement`Класс является производным от `StringElement` , но позволяет разработчикам настраивать несколько свойств, таких как шрифт, цвет текста, цвет ячейки фона, режим разрыва строки, число отображаемых строк и необходимость отображения принадлежности.

### <a name="multiline-element"></a>Многострочный элемент

 [![Многострочный элемент](images/image10.png)](images/image10.png#lightbox)

### <a name="entry-element"></a>Элемент Entry
...
`EntryElement`, Как подразумевается имя, используется для получения вводимых пользователем данных. Он поддерживает обычные строки или пароли, в которых символы скрыты.
 ...
 [![Ентрелемент используется для получения вводимых пользователем данных](images/image11.png)](images/image11.png#lightbox)
...
### <a name="boolean-element"></a>Логический элемент
 ...
 [![Логический элемент](images/image12.png)](images/image12.png#lightbox)

### <a name="checkbox-element"></a>Элемент CheckBox

 [![Элемент CheckBox](images/image13.png)](images/image13.png#lightbox)

### <a name="radio-element"></a>Элемент Radio

`RadioElement` `RadioGroup` Для необходимо указать объект в `RootElement` .

```csharp
mtRoot = new RootElement ("Demos", new RadioGroup("MyGroup", 0));
```

 [![Для Радиоелемент требуется указать RadioGroup в RootElement](images/image14.png)](images/image14.png#lightbox)
...
Имя группы в используется `RadioGroup` для отображения выбранного значения на содержащей странице (если таковое имеется) и в этом случае значение, равное нулю, — индекс первого выбранного элемента.

### <a name="badge-element"></a>Элемент эмблемы

 [![Элемент эмблемы](images/image15.png)](images/image15.png#lightbox)

### <a name="float-element"></a>Float, элемент

 [![Float, элемент](images/image16.png)](images/image16.png#lightbox)

### <a name="activity-element"></a>Элемент Activity

 [![Элемент Activity](images/image17.png)](images/image17.png#lightbox)

### <a name="date-element"></a>Элемент Date

 ![Элемент Date](images/image18.png)

Когда выбрана ячейка, соответствующая Датилемент, отображается элемент выбора даты, как показано ниже:

 [![Если выбрана ячейка, соответствующая Датилемент, отображается элемент выбора даты, как показано](images/image19.png)](images/image19.png#lightbox)

### <a name="time-element"></a>Элемент Time

 [![Элемент Time](images/image20.png)](images/image20.png#lightbox)

Когда выбрана ячейка, соответствующая Тимилемент, отображается средство выбора времени, как показано ниже:

 [![Когда выбрана ячейка, соответствующая Тимилемент, отображается средство выбора времени, как показано.](images/image21.png)](images/image21.png#lightbox)

### <a name="datetime-element"></a>Элемент DateTime

 [![Элемент DateTime](images/image22.png)](images/image22.png#lightbox)

Если выбрана ячейка, соответствующая Датетимилемент, отображается средство выбора даты и времени, как показано ниже:

 [![Если выбрана ячейка, соответствующая Датетимилемент, отображается средство выбора даты и времени.](images/image23.png)](images/image23.png#lightbox)

### <a name="html-element"></a>HTML-элемент

 [![HTML-элемент](images/image24.png)](images/image24.png#lightbox)

`HTMLElement`Отображает значение его `Caption` свойства в ячейке таблицы. Вхе, `Url` назначенный элементу, загружается в `UIWebView` элемент управления, как показано ниже:

 [![Вхе выбрано, URL-адрес, назначенный элементу, загружается в элемент управления Уивебвиев, как показано ниже.](images/image25.png)](images/image25.png#lightbox)

### <a name="message-element"></a>Элемент Message

 [![Элемент Message](images/image26.png)](images/image26.png#lightbox)
...
`UIActivity`Индикатор начинает анимацию, а заголовок загрузки отображается, когда пользователь касается ячейки, а затем `NSAction` выполняется переданный в конструктор. После завершения выполнения кода в элементе `NSAction` `UIActivity` индикатор будет остановлен, а стандартный заголовок появится снова.

### <a name="uiview-element"></a>UIView, элемент

Кроме того, любое пользовательское `UIView` значение можно отобразить с помощью `UIViewElement` .

### <a name="owner-drawn-element"></a>Рисуемый владельцем элемент

Этот элемент должен быть подклассом, так как он является абстрактным классом. Следует переопределить `Height(RectangleF bounds)` метод, в котором следует возвращать высоту элемента, а также `Draw(RectangleF bounds, CGContext context, UIView view)` все пользовательские рисунки в пределах заданных границ, используя параметры контекста и представления. Этот элемент выполняет тяжелую работу с подклассом a `UIView` и помещением его в возвращаемую ячейку, при этом требуется только реализовать два простых переопределения. В примере приложения в файле можно увидеть лучшую демонстрационную реализацию `DemoOwnerDrawnElement.cs` .
...
`JsonElement`— Это подкласс `RootElement` , расширяющий возможности `RootElement` для загрузки содержимого вложенного дочернего элемента с локального или удаленного URL-адреса.

`JsonElement`— Это объект `RootElement` , который можно создать в двух формах. Одна версия создает `RootElement` , которая будет загружать содержимое по запросу. Они создаются с помощью `JsonElement` конструкторов, которые принимают дополнительный аргумент в конце, URL-адрес для загрузки содержимого:
...
Дополнительные сведения об использовании JSON с MT. Г. см. [Пошаговое руководство по элементу JSON](./json-element-walkthrough.md) .
...
Чтобы добавить в диалоговых окнах автоматическую поддержку обновления по запросу, необходимо выполнить два действия: подключить обработчик событий, чтобы получать уведомления, когда пользователь извлекает данные и уведомляет о `DialogViewController` загрузке данных, чтобы вернуться к состоянию по умолчанию.

Присоединиться к уведомлению очень просто; Просто подключитесь к `RefreshRequested` событию в `DialogViewController` , например:

```csharp
dvc.RefreshRequested += OnUserRequestedRefresh;
```

Затем в методе `OnUserRequestedRefresh` вы заставите в очередь какую-либо загрузку данных, запросите некоторые данные из сети или прокрутить поток, чтобы вычислить данные. После загрузки данных необходимо уведомить о том `DialogViewController` , что новые данные находятся в, и восстановить представление в состояние по умолчанию, вызвав `ReloadComplete` :
...
Для поддержки поиска задайте `EnableSearch` свойство в `DialogViewController` . Можно также задать `SearchPlaceholder` свойство для использования в качестве текста водяного знака в строке поиска.
...
Загрузчик изображений реализуется в `ImageLoader` классе, все, что нужно сделать — это вызвать `DefaultRequestImage` метод, необходимо предоставить URI для загружаемого образа, а также экземпляр `IImageUpdated` интерфейса, который будет вызываться при загрузке изображения.
...
Благодаря разумному использованию синтаксиса инициализации LINQ и C# можно использовать LINQ для создания иерархии элементов. Например, следующий код создает экран из некоторых массивов строк и обрабатывает выделение ячеек с помощью анонимной функции, передаваемой в каждую из них `StringElement` :
...
И отражение, и API элементов используют одно и то же `DialogViewController` . Иногда требуется настроить внешний вид представления или использовать некоторые функции `UITableViewController` , которые выходят за пределы базового создания пользовательских интерфейсов.

`DialogViewController`Является просто подклассом, `UITableViewController` и его можно настроить так же, как и при настройке `UITableViewController` .
...
Если вы хотите выполнить проверку данных, это следует делать, когда пользователь запускает действие с введенными данными. Например, кнопка **Готово** или **Далее** на верхней панели инструментов или другая кнопка `StringElement` для перехода к следующему этапу.
...
В этой статье мы рассмотрели множество сведений о том, как было бы очень Touch. В нем обсуждались фундаментальные сведения о том, как MT. D работает и охватывает различные компоненты, составляющие MT. D. Он также показал широкий массив элементов и настроек таблиц, поддерживаемых MT. D и обсуждалось, как MT. D можно расширить с помощью настраиваемых элементов. Кроме того, в нем объясняется поддержка JSON в MT. D, позволяющий динамически создавать элементы из JSON.
