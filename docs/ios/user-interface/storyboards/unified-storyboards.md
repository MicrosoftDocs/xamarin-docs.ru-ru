...
Когда контроллер разделенного представления в приложении адаптивных фотографий переходит из свернутого в развернутый, в анимацию по умолчанию добавляются анимации путем переопределения `WillTransitionToTraitCollection` метода контроллера представления. Например.
