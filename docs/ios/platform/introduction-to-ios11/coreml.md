...
Эти шаги описывают добавление CoreML в проект iOS. Практический пример см. в [образце Habitatного расценок для Mars](/samples/xamarin/ios-samples/ios12-marshabitatcoremltimer/) .
...
Параметры модели передаются и исправляются с помощью класса контейнера, реализующего интерфейс `IMLFeatureProvider` .
...
Результат прогноза `outFeatures` также является экземпляром `IMLFeatureProvider` ; к выходным значениям можно обращаться с использованием `GetFeatureValue` имени каждого выходного параметра (например `theResult` ,), как показано в следующем примере:
...
Описанные ниже действия описывают, как CoreML и концепция используются вместе в [примере коремлвисион](/samples/xamarin/ios-samples/ios11-coremlvision). Этот пример сочетает [Распознавание прямоугольников](~/ios/platform/introduction-to-ios11/vision.md#rectangles) от концепции инфраструктуры с моделью _мнинстклассифиер_ CoreML для идентификации рукописной цифры на фотографии.
...
Классу по-прежнему требуется реализовать `HandleRectangles` `HandleClassification` методы и для запросов к концепции, показанные в шагах 3 и 4 ниже.
...
`request`Параметр, переданный в этот метод, содержит подробные сведения о запросе концепции и `GetResults<VNRectangleObservation>()` метод, который возвращает список прямоугольников, найденных в изображении. Первый прямоугольник `observations[0]` извлекается и передается в модель CoreML:
...
`ClassificationRequest`Был инициализирован на шаге 1 для использования `HandleClassification` метода, определенного на следующем шаге.

### <a name="4-handle-the-coreml"></a>4. Обработайте CoreML

`request`Параметр, переданный в этот метод, содержит сведения о запросе CoreML и `GetResults<VNClassificationObservation>()` метод, который возвращает список возможных результатов, упорядоченных по достоверности (с наибольшей достоверностью):
...
- [Образец прогнозируемого Habitat цен на Mars](/samples/xamarin/ios-samples/ios12-marshabitatcoremltimer/) имеет простые числовые входные и выходные данные.

- [Образец & CoreML "концепция](/samples/xamarin/ios-samples/ios11-coremlvision) " принимает параметр Image и использует инфраструктуру видения для идентификации квадратных областей в изображении, которые передаются в модель CoreML, распознающую одиночные цифры.

- Наконец, [Пример распознавания изображений CoreML](/samples/xamarin/ios-samples/ios11-coremlimagerecognition) использует CoreML для поиска компонентов в фотографии. По умолчанию используется меньшая модель **скуизенет** (5 МБ), но она была написана так, чтобы можно было загрузить и внедрить более крупную модель **VGG16** (553MB). Дополнительные сведения см. в [файле сведений для примера](https://github.com/xamarin/ios-samples/blob/master/ios11/CoreMLImageRecognition/CoreMLImageRecognition/README.md).

## <a name="related-links"></a>Связанные ссылки

- [Машинное обучение (Apple)](https://developer.apple.com/machine-learning/)
- [Пример CoreML (режим MARS Habitat) (пример)](/samples/xamarin/ios-samples/ios12-marshabitatcoremltimer/)
- [CoreML и концепция (Распознавание номеров) (пример)](/samples/xamarin/ios-samples/ios11-coremlvision)
- [Распознавание изображений CoreML (пример)](/samples/xamarin/ios-samples/ios11-coremlimagerecognition)
- [CoreML с Пользовательское визуальное распознавание Azure (пример)](/samples/xamarin/ios-samples/ios11-coremlazuremodel)
