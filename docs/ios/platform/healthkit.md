...
- Объект `Entitlements.plist` со `com.apple.developer.healthkit` свойством типа, `Boolean` для которого задано значение `Yes` .
- Объект `Info.plist` , `UIRequiredDeviceCapabilities` ключ которого содержит запись со `String` значением `healthkit` .
- `Info.plist`Кроме того, необходимо иметь соответствующие записи с пояснениями о конфиденциальности: `String` Описание ключа, `NSHealthUpdateUsageDescription` Если приложение будет записывать данные, и `String` Описание ключа, `NSHealthShareUsageDescription` Если приложение будет считывать данные набора работоспособности.
...
После создания и установки соответствующего **профиля подготовки** , как описано выше, обычно пришло время создать решение в Visual Studio для Mac или Visual Studio. Доступ к набору работоспособности доступен для любого проекта iOS C# или F #.
...
Если **профиль подготовки** недоступен, дважды проверьте **идентификатор пакета** на панели **приложений iOS** , а также в **центре разработки iOS** и убедитесь, что **профиль подготовки** установлен (**Xcode > предпочтения > учетные записи > просмотра сведений...**).
...
Пример приложения включает `Entitlements.plist` файл (который необходим для приложений с поддержкой пакета работоспособности) и не включается в каждый шаблон проекта. Если проект не содержит прав, щелкните правой кнопкой мыши проект, выберите **файл > создать файл... > iOS > прав. plist** , чтобы добавить его вручную.
...
Аналогично, `Info.plist` для приложения должно быть связано значение, `healthkit` связанное с `UIRequiredDeviceCapabilities` ключом:
...
Пример приложения, приведенный в этой статье, включает в себя предварительно настроенный `Entitlements.plist` , включающий все необходимые ключи.
...
Данные набора работоспособности ограничены указанными в Apple типами. Эти типы строго определены: некоторые, например тип крови, ограничиваются определенными значениями перечисления, предоставляемого компанией Apple, а другие сочетают величину с единицей измерения (например, граммы, калориес и литры). Даже данные, совместно использующие совместимую единицу измерения, различны `HKObjectType` . Например, система типов будет перехватывать ошибочную попытку сохранить `HKQuantityTypeIdentifier.NumberOfTimesFallen` значение в поле, ожидающем, несмотря на то `HKQuantityTypeIdentifier.FlightsClimbed` , что оба используют `HKUnit.Count` единицу измерения.
...
`HKQuantityType`типы находятся в диапазоне от `HKQuantityTypeIdentifier.ActiveEnergyBurned` до `HKQuantityTypeIdentifier.StepCount` . 
...
В проекте, подготовленном для набора работоспособности, откройте `AppDelegate.cs` файл. Обратите внимание на инструкцию `HealthKit` , используя, в начале файла.
...
Весь код в этих методах можно сделать встроенным `OnActivated` , но пример приложения использует отдельные методы, чтобы сделать их более четкими: `ValidateAuthorization()` содержит шаги, необходимые для запроса доступа к заданным типам (и для чтения, если требуется приложение) и `ReactToHealthCarePermissions()` является обратным вызовом, который активируется после того, как пользователь взаимодействовал с диалоговым окном разрешений в Health. app.

Задание `ValidateAuthorization()` — это создание набора `HKObjectTypes` , в котором приложение будет записывать и запрашивать авторизацию для обновления этих данных. В примере приложения параметр `HKObjectType` предназначен для ключа `KHQuantityTypeIdentifierKey.HeartRate` . Этот тип добавляется в набор `typesToWrite` , в то время как набор `typesToRead` остается пустым. Эти наборы и ссылка на `ReactToHealthCarePermissions()` обратный вызов передается в `HKHealthStore.RequestAuthorizationToShare()` .

`ReactToHealthCarePermissions()`Обратный вызов будет вызываться после того, как пользователь взаимодействовал с диалоговым окном "разрешения" и передавал два фрагмента информации: `bool` значение, которое будет иметь в `true` виду, если пользователь взаимодействовал с диалоговым окном разрешений и `NSError` что, если не равно null, указывает на ошибку, связанную с диалоговым окном разрешений.
...
`HeartRateInBeatsPerMinute()`Преобразует свой аргумент в набор работоспособности со строгим типом `HKQuantity` . Тип количества определяется параметром, `HKQuantityTypeIdentifierKey.HeartRate` а единицы измерения `HKUnit.Count` делятся на `HKUnit.Minute` (иными словами, единица *перебивается в минуту*). 

`StoreHeartRate()`Функция принимает объект `HKQuantity` (в примере приложения, созданный с помощью `HeartRateInBeatsPerMinute()` ). Для проверки данных используется `HKQuantity.IsCompatible()` метод, который возвращает значение, `true` Если единицы объекта можно преобразовать в единицы в аргументе. Если количество было создано с помощью `HeartRateInBeatsPerMinute()` этого `true` , очевидно, возвращается значение, но оно также будет возвращать, `true` Если количество было создано как, например, в *секунду в час*. Более часто `HKQuantity.IsCompatible()` можно использовать для проверки массы, расстояния и энергии, которые пользователь или устройство могут вводить или отображать в одной системе измерения (например, германской Units), но они могут храниться в другой системе (например, в единицах метрик). 

После проверки совместимости количества `HKQuantitySample.FromType()` используется метод фабрики для создания строго типизированного `heartRateSample` объекта. `HKSample`объекты имеют дату начала и окончания; для мгновенного считывания эти значения должны быть одинаковыми, как в примере. В этом примере также не задаются никакие данные ключа-значения в `HKMetadata` аргументе, но для указания расположения датчика можно использовать следующий код:
...
После `heartRateSample` создания код создает новое соединение с базой данных с помощью блока using. В этом блоке `HKHealthStore.SaveObject()` метод выполняет попытку асинхронной записи в базу данных. Результирующий вызов лямбда-выражения вызывает соответствующие события: `HeartRateStored` или `ErrorMessageChanged` .

Теперь, когда модель была запрограммирована, пришло время увидеть, как контроллер отражает состояние модели. Откройте `HKWorkViewController.cs` файл. Конструктор просто подразделяет `HeartRateModel` одноэлементный класс на методы обработки событий (опять же, это можно сделать с помощью лямбда-выражений, но отдельные методы делают цель немного более очевидна):
...
Разрешите приложению обновлять данные о частоте сердца, и ваше приложение появится снова. `ReactToHealthCarePermissions`Обратный вызов будет активирован асинхронно. Это приведет к `HeartRateModel’s` `Enabled` изменению свойства, которое вызовет `EnabledChanged` событие, что приведет к `HKPermissionsViewController.OnEnabledChanged()` запуску обработчика событий, что активирует `StoreData` кнопку. Последовательность показана на следующей схеме:

[![](healthkit-images/image13.png "This diagram shows the sequence of events")](healthkit-images/image13.png#lightbox)

Нажмите кнопку **запись** . Это приведет к `StoreData_TouchUpInside()` запуску обработчика, который попытается проанализировать значение `heartRate` текстового поля, преобразовать в объект с `HKQuantity` помощью ранее обсуждаемой `HeartRateModel.HeartRateInBeatsPerMinute()` функции и передать это количество в `HeartRateModel.StoreHeartRate()` . Как обсуждалось ранее, в этом случае будет предпринята попытка сохранить данные и будет вызвано `HeartRateStored` `ErrorMessageChanged` событие или.
...
Чтение данных из базы данных комплекта работоспособности очень похоже на запись данных: одна из них указывает типы данных, к которым пытается получить доступ, запросы авторизации, а также, если авторизация предоставлена, данные доступны с автоматическим преобразованием в совместимые единицы измерения.
...
Мы также показали, как конфиденциальность, безопасность и целостность данных переопределяют проблемы, связанные со сведениями о работоспособности, и приложения, использующие набор средств работоспособности, должны справляться с повышением сложности в аспектах управления приложениями (подготовка), программировании (система типов пакета работоспособности) и пользовательском интерфейсе (Управление разрешениями с помощью системных диалоговых окон и приложения работоспособности). 
