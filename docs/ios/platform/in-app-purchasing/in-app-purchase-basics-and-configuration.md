...
Вы можете использовать любую строку для **идентификатора пакета** (при условии, что он уникален в вашей учетной записи), однако Apple рекомендует использовать формат обратных DNS, а не любую произвольную строку. В образце приложения, прилагаемом к этой статье, используется COM. Xamarin. storekit. testing для идентификатора пакета, однако было бы так же допустимо использовать идентификатор, такой как my_store_example (даже если Apple не рекомендует его).
