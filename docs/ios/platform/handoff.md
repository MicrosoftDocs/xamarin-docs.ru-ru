...
`ContinueUserActivity`Метод включает в себя объект `UIApplicationRestorationHandler` , который можно вызывать для возобновления действия на основе документа или респондента. `NSArray`При вызове в обработчик восстановления необходимо передать объекты или restorable. Например.

```csharp
completionHandler (new NSObject[]{Tab4});
```

Для каждого переданного объекта `RestoreUserActivityState` будет вызван его метод. Затем каждый объект может использовать данные в `UserInfo` словаре для восстановления своего собственного состояния. Например.
...
`openssl`Команда выводит подписанный файл JSON, который вы поместите на веб-сайт, по URL-адресу **Apple-App-site-Association** . Например.
...
Исходное приложение задаст `SupportsContinuationStreams` свойству `NSUserActivity` экземпляра значение `true` . Например.
...
Принимающее приложение может затем вызвать `GetContinuationStreams` метод класса в, `NSUserActivity` `AppDelegate` чтобы установить поток. Например.
