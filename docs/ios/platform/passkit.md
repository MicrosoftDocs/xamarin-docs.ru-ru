...
В этом документе описывается бумажник и использование API PassKit с Xamarin. iOS.
 ...
 [![Бумажник хранит и упорядочивает все этапы на телефоне.](passkit-images/image1.png)](passkit-images/image1.png#lightbox)
...
PassKit — не просто API в Кокоатауч, он является частью более крупной экосистемы приложений, данных и служб, которые упрощают безопасный общий доступ и управление штрихкодами и другими данными. На этой высокоуровневой схеме показаны различные сущности, которые могут быть задействованы при создании и использовании проходов:
 ...
 [![На этой высокоуровневой схеме показаны сущности, участвующие в создании и использовании проходов.](passkit-images/image2.png)](passkit-images/image2.png#lightbox)
...
На этом снимке экрана показаны пять типов Pass (в порядке: купон, универсальный, карта магазина, проход на доске и билет события):
 ...
 [![На этом снимке экрана показаны пять типов Pass.](passkit-images/image3.png)](passkit-images/image3.png#lightbox)
...
- **pass.js** — обязательно. Содержит все сведения для прохода.
- **manifest.js** — обязательно. Содержит хэши SHA1 для каждого файла в проходе, за исключением файла сигнатуры и этого файла (manifest.jsв).
- **подпись** — обязательное. Создается путем подписи  `manifest.json` файла сертификатом, созданным на портале подготовки iOS.
- **logo.png** — необязательный.
...
Структура каталогов передаваемого файла показана ниже (содержимое ZIP-архива):
 ...
 [![Структура каталогов транзитного файла показана здесь](passkit-images/image4.png)](passkit-images/image4.png#lightbox)

### <a name="passjson"></a>pass.jsна
...
- **SerialNumber** — уникальная строка в этом объекте  `passTypeidentifier` . Значение является непрозрачным для бумажника, но важно для отслеживания конкретных проходов при взаимодействии с сервером.
...
1. Напишите в  `manifest.json` виде списка ключей и значений JSON каждого имени файла с его хэшем.
...
1. В  [разделе Сертификаты, идентификаторы и профили на портале подготовки iOS](https://developer.apple.com/account/overview.action)перейдите в раздел  **идентификаторы** и выберите  **тип передачи идентификаторов** . Затем нажмите **+** кнопку, чтобы создать новый тип передачи: [ ![ создать новый тип передачи](passkit-images/passid.png)](passkit-images/passid.png#lightbox)

2. Введите **Описание** (имя) и **идентификатор** (уникальную строку) для прохода. Обратите внимание, что все идентификаторы типа Pass должны начинаться со строки `pass.` в этом примере `pass.com.xamarin.coupon.banana` : [ ![ Введите описание и идентификатор](passkit-images/register.png)](passkit-images/register.png#lightbox) .
...
1. Выберите только что созданный идентификатор прохода из списка и нажмите кнопку **изменить** : [ ![ выберите новый идентификатор Pass из списка](passkit-images/pass-done.png)](passkit-images/pass-done.png#lightbox) .

    Затем выберите  **создать сертификат...** :

    [![Выберите создать сертификат.](passkit-images/cert-dist.png)](passkit-images/cert-dist.png#lightbox)
...
- Создайте pass.jsдля файла, содержащего все необходимые данные.
- Включить изображения в папку (при необходимости).
- Вычислите хэши SHA1 для каждого файла в папке и запишите их в manifest.js.
- Подпишитесь manifest.jsс скачанным файлом Certificate. p12.
- Заархивировать содержимое каталога и переименовать его с расширением. пкпасс.

В [образце кода](/samples/xamarin/ios-samples/passkit) для этой статьи есть некоторые исходные файлы, которые можно использовать для создания прохода. Используйте файлы в `CouponBanana.raw` каталоге каталога креатеапассмануалли. Имеются следующие файлы:

 [![Эти файлы существуют](passkit-images/image18.png)](passkit-images/image18.png#lightbox)

Откройте pass.jsна и измените JSON. Необходимо по крайней мере обновить `passTypeIdentifier` и, `teamIdentifer` чтобы соответствовать вашей учетной записи разработчика Apple.
...
Загрузите **материалы о поддержке начального значения бумажника** на сайте [загрузки Apple](https://developer.apple.com/downloads/index.action?name=Passbook) . Используйте `signpass` средство, чтобы превратить папку в проход (при этом будут также вычислены ХЭШИ SHA1 и ЗАархивировать выходные данные в пкпасс-файл).

#### <a name="testing"></a>Тестирование

Если вы изучите выходные данные этих средств (задав для файла имя. zip, а затем открыли его), вы увидите следующие файлы (Обратите внимание на добавление `manifest.json` `signature` файлов и):

 [![Проверка выходных данных этих средств](passkit-images/image19.png)](passkit-images/image19.png#lightbox)
...
После подписания ZIP-файла и его переименования (например, в `BananaCoupon.pkpass` ) вы можете перетащить его в симулятор, чтобы протестировать, или отправить его самому себе для получения на реальном устройстве. Вы увидите экран, чтобы **Добавить** этот проход, как показано ниже.
 ...
 [![Добавление экрана Pass](passkit-images/image20.png)](passkit-images/image20.png#lightbox)
...
Бумажник — Центральная часть экосистемы PassKit. На этом снимке экрана показан пустой бумажник, а также показано, как выглядит список пройденных и отдельных проходов:
 ...
 [![На этом снимке экрана показан пустой бумажник, а также показано, как выглядит список пройденных и отдельных проходов](passkit-images/image21.png)](passkit-images/image21.png#lightbox)
...
На этом снимке экрана показано, как **почта** в iOS 6 распознает вложенное вложение и (при его наличии) предложение **Добавить** его в бумажник.
 ...
 [![На этом снимке экрана показано, как почта в iOS 6 распознает вложенное вложение](passkit-images/image22.png)](passkit-images/image22.png#lightbox)

 [![На этом снимке экрана показано, как почта предлагает добавить вложенное вложение в бумажник.](passkit-images/image23.png)](passkit-images/image23.png#lightbox)
...
![Права. редактор плст](passkit-images/image31.png)

В разделе "Бумажник" выберите параметр **включить бумажник** .

![Включить право на бумажник](passkit-images/image32.png)
...
Чтобы добавить право на бумажник, задайте для **Свойства** значение `Passbook Identifiers` в раскрывающемся списке, который автоматически установит **тип** `Array` . Затем задайте строковое **значение** `$(TeamIdentifierPrefix)*` :

![Включить право на бумажник](passkit-images/image33.png)
...
в этом случае `pass-type-identifiers` массив прав неверен (или не соответствует **профилю подготовки**). Проверьте правильность идентификаторов типа Pass и идентификатора команды.
...
См. Проект Пасслибрари в [образце кода](/samples/xamarin/ios-samples/passkit) для этой статьи. В нем демонстрируются следующие общие функции, которые потребуются в приложении-компаньоне бумажника:
...
Обратите внимание, что симулятор не фильтрует список возвращаемых проходов, поэтому этот метод всегда должен быть протестирован на реальных устройствах. Этот список можно отобразить в Уитаблевиев. После добавления двух купонов [пример приложения](/samples/xamarin/ios-samples/passkit) выглядит следующим образом:

 [![Пример приложения выглядит следующим образом после добавления двух купонов](passkit-images/image29.png)](passkit-images/image29.png#lightbox)
...
Эта строка отображается как предупреждение в [примере](/samples/xamarin/ios-samples/passkit):

 [![Выбранный купон оповещение в образце](passkit-images/image30.png)](passkit-images/image30.png#lightbox)
...
Этот проход представлен с помощью параметров **Add** и **Cancel** :
 ...
 [![Передача, представленная с помощью параметров Add и Cancel](passkit-images/image20.png)](passkit-images/image20.png#lightbox)
...
См. раздел [DotNet-расчетной книжки](https://github.com/tomasmcguinness/dotnet-passbook) с открытым кодом C# на стороне сервера.
...
- [Пример PassKit](/samples/xamarin/ios-samples/passkit)
