...
Можно легко проверить, есть ли у данного контакта требуемое свойство, с помощью `IsKeyAvailable` `AreKeysAvailable` методов или `CNContact` экземпляра. Например.
